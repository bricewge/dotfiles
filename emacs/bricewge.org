...
*** VCSH acces via TRAMP
Hack, to access VCSH repo through TRAMP. Example, open the file
=/vcsh:zsh:.= and then do =M-x= =magit-status=.
#+BEGIN_SRC emacs-lisp
...
(defvar vcsh-repo-d
;; Wasn't been "VCSH_REPO_D"
  (or (getenv "VCSH_HOOK_D")
      (substitute-in-file-name "$HOME/.config/vcsh/repo.d")))
;;      (substitute-in-file-name "$XDG_CONFIG_HOME/.config/vcsh/repo.d")))
...
;;                          (directory-files "$VCSH_REPO_D/" nil "^[^.]")
